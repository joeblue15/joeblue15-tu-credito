    ...
    if (this.state.hasError) {
      return (
        <div className="rounded-[32px] border border-red-500/20 bg-red-500/5 p-8 text-center">
          <h1 className="text-2xl font-semibold text-foreground">Algo falló al renderizar</h1>
          <p className="mt-2 text-sm text-red-200">{this.state.message || "Error desconocido"}</p>
          <button
            type="button"
            onClick={() => this.setState({ hasError: false, message: undefined })}
            className="mt-5 rounded-full border border-border bg-card px-4 py-2 text-sm text-foreground hover:bg-accent"
          >
            Reintentar
          </button>
        </div>

  ...
  return (
    <article className="surface p-4 md:p-5">
      ...
      <div className="mt-6 grid grid-cols-2 gap-2">
        ...
        <Button
          variant="outline"
          className="btn-189 h-9 rounded-none border-border bg-transparent text-[11px] font-medium text-foreground hover:bg-transparent hover:text-foreground"
      ...
      <Button
        variant="ghost"
        className="btn-189 mt-3 h-8 w-full rounded-none text-[11px] text-muted-foreground hover:text-foreground"
        onClick={() => toggleFavorite(card.slug)}
      >
        {isFavorite(card.slug) ? "♥ Guardada" : "♡ Guardar"}
      </Button>
    </article>
  );
}
